...
class MyAdvertisedDeviceCallbacks: public BLEAdvertisedDeviceCallbacks {
    ...
    boolean checkForBluetoothSensorAddress(BLEAddress checkAddress)
    {
      ...
      for (i = 0; i < MAXBLUETOOTHDEVICES; i++)
      {
        ...
        {
          /*Serial.print("BTA[i]=");
          Serial.println(BLEAddress(BluetoothAddresses[i].c_str()));
          Serial.print("checkAddress=");
          Serial.println(checkAddress);
          */
          if ( BLEAddress(BluetoothAddresses[i].c_str()) == checkAddress)
          {
            Serial.println("*******BLE Address Found********");
            return true;
          }
        }
      }
    ...
    void onResult(BLEAdvertisedDevice * advertisedDevice) {
      ...
      */
      if (checkForBluetoothSensorAddress(advertisedDevice->getAddress()))
      {

        Serial.printf("BLE Assigned Sensor Device found: \n%s\n", advertisedDevice->toString().c_str());
        ...
        DEBUG_PRINT("Address Not found on this BLE scan.\n");
    } // onResult
...
/*
  ...
  //Serial.println("After each read  - reinitializing");
